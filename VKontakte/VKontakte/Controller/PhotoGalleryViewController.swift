...
//Запущена ли анимация в протиположную сторону?  Если да,  то делаем ее реверс
                   if isRightAnimatorStarted{
                     ...
                     case .began:
                         ...
                         if photoIndex == friendPhoto.count - 1 {
                         ...
                         propertyAnimatorLeft.addCompletion { position in
                             switch position {
                             case .end:
                                 guard self.photoIndex != self.friendPhoto.count - 1 else {break}
                     ...
                     case .ended:
                         ...
                         guard photoIndex != friendPhoto.count - 1 else{
    ...
    func setupItems(){
        currentView.image = friendPhoto[photoIndex]?.photo
